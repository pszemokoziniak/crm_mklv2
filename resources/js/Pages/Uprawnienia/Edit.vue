<template>
  <div>
    ...
    <h1 class="mb-8 text-3xl font-bold">
      <Link class="text-indigo-400 hover:text-indigo-600" href="/branza">Zakres</Link>
    ...
    <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="update">
        ...
        <div class="flex items-center px-8 py-4 bg-gray-50 border-t border-gray-100">
          <button v-if="!zakres.deleted_at" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy">Usuń</button>
  ...
  data() {
    return {
      form: this.$inertia.form({
        id: this.zakres.id,
        name: this.zakres.name,
      }),
    }
  },
  methods: {
    update() {
      this.form.post(`/zakres/${this.zakres.id}`)
    },
    destroy() {
      if (confirm('Czy aby napewno?')) {
        this.$inertia.delete(`/zakres/${this.zakres.id}`)
      }
    },
    restore() {
      if (confirm('Chcesz przywrócić rekord?')) {
        this.$inertia.put(`/zakres/${this.zakres.id}/restore`)
      }
    },
  },
}
