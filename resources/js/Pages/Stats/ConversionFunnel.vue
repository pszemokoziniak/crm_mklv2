    ...
    <div class="p-4">
      <div class="space-y-3 max-w-3xl mx-auto">
        ...
        <div class="relative">
          <div class="flex items-center gap-3">
            <div class="w-40 flex-shrink-0 text-right">
              <div class="text-xs font-bold uppercase text-gray-500">Z ofertą</div>
              <div class="text-[10px] text-gray-400">zapytania z >=1 ofertą</div>
            </div>
        ...
        <!-- Stage 3: Oferty (total) -->
        <div class="relative">
          <div class="flex items-center gap-3">
            <div class="w-40 flex-shrink-0 text-right">
              <div class="text-xs font-bold uppercase text-gray-500">Oferty złożone</div>
              <div class="text-[10px] text-gray-400">wszystkie w okresie</div>
            </div>
            <div class="flex-1 relative h-14 rounded-lg overflow-hidden bg-gray-100 flex items-center">
              <div class="h-full flex items-center justify-center text-white font-bold shadow-inner"
                :style="{ width: barWidth(funnel.oferty_total, funnel.zapytania_total) + '%', background: 'linear-gradient(90deg, #0d9488, #14b8a6)' }">
                {{ funnel.oferty_total }}
              </div>
            </div>
            <div class="w-20 text-right">
              <div class="text-[10px] text-gray-400">wolumen ofert</div>
            </div>
          </div>
        </div>
        ...
        <div class="relative">
          <div class="flex items-center gap-3">
            <div class="w-40 flex-shrink-0 text-right">
              <div class="text-xs font-bold uppercase text-gray-500">Wygrane</div>
              <div class="text-[10px] text-gray-400">status = Wygrana</div>
            </div>
            <div class="flex-1 relative h-14 rounded-lg overflow-hidden bg-gray-100 flex items-center">
              <div class="h-full flex items-center justify-center text-white font-bold shadow-inner"
                :style="{ width: barWidth(funnel.oferty_wygrane, funnel.zapytania_total) + '%', background: 'linear-gradient(90deg, #059669, #10b981)' }">
                {{ funnel.oferty_wygrane }}
              </div>
            </div>
            <div class="w-20 text-right">
              <div class="text-lg font-bold text-green-600">{{ funnel.pct_oferty_to_wygrana }}%</div>
              <div class="text-[9px] text-gray-400">z ofert</div>
            </div>
          </div>
        </div>
      </div>

      <!-- Rozklad ofert per status -->
      <div class="max-w-3xl mx-auto mt-8 pt-6 border-t border-gray-100">
        <h3 class="text-xs font-bold uppercase text-gray-500 mb-3 text-center">Rozkład ofert (w okresie)</h3>
        <div class="grid grid-cols-3 gap-3 text-center">
          <div class="bg-green-50 rounded-lg p-3 border border-green-100">
            <div class="text-[10px] uppercase font-bold text-green-600">Wygrane</div>
            <div class="text-xl font-bold text-green-700">{{ funnel.oferty_wygrane }}</div>
            <div class="text-[10px] text-gray-500">{{ funnel.pct_oferty_to_wygrana }}%</div>
          </div>
          <div class="bg-blue-50 rounded-lg p-3 border border-blue-100">
            <div class="text-[10px] uppercase font-bold text-blue-600">Toczy się</div>
            <div class="text-xl font-bold text-blue-700">{{ funnel.oferty_toczy }}</div>
            <div class="text-[10px] text-gray-500">{{ pctOfertyToToczy }}%</div>
          </div>
          <div class="bg-rose-50 rounded-lg p-3 border border-rose-100">
            <div class="text-[10px] uppercase font-bold text-rose-600">Przegrane</div>
            <div class="text-xl font-bold text-rose-700">{{ funnel.oferty_przegrane }}</div>
            <div class="text-[10px] text-gray-500">{{ funnel.pct_oferty_to_przegrana }}%</div>
          </div>
        </div>
      </div>
    </div>
