<template>
  <div>
    ...
    <trashed-message v-if="faza.deleted_at" class="mb-6" @restore="restore"> This faza has been deleted. </trashed-message>
    <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="update">
        ...
        <div class="flex items-center px-8 py-4 bg-gray-50 border-t border-gray-100">
          <button v-if="!faza.deleted_at" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy">Usuń</button>
...
export default {
  ...
  methods: {
    update() {
      this.form.post(`/faza/${this.faza.id}`)
    },
    destroy() {
      if (confirm('Czy aby napewno?')) {
        this.$inertia.delete(`/faza/${this.faza.id}`)
      }
    },
    restore() {
      if (confirm('Chcesz przywrócić rekord?')) {
        this.$inertia.put(`/faza/${this.faza.id}/restore`)
      }
    },
  },
}
