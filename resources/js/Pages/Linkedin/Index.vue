<template>
  <div>
    <Head title="LinkedIn" />
    <h1 class="mb-8 text-3xl font-bold">LinkedIn</h1>
      ...
      <table class="w-full table-auto">
        <thead>
          <tr class="text-left font-bold bg-gray-50">
            <th class="pb-4 pt-6 px-2 sm:px-6">Firma</th>
        ...
        <tbody>
          <tr v-for="item in linkedin.data" :key="item.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
            <td class="border-t px-2 sm:px-6 py-4">
              <Link class="flex items-center focus:text-indigo-500" :href="`/linkedin/${item.id}/edit`">
                <span class="font-medium break-words">{{ item.client?.nazwa || 'Brak klienta' }}</span>
            ...
            <td class="border-t px-2 sm:px-6 py-4 hidden md:table-cell">
              <Link class="flex items-center text-xs whitespace-nowrap" :href="`/linkedin/${item.id}/edit`">
                {{ item.updated_at }}
              </Link>
            </td>
            <td class="border-t px-2 sm:px-6 py-4 text-center">
              <Link class="block text-sm" :href="`/linkedin/${item.id}/edit`">
                {{ item.click }}
              </Link>
            </td>
            <td class="border-t px-2 sm:px-6 py-4 text-right">
              <div class="flex items-center justify-end">
                <a class="btn-indigo py-1 px-2 sm:px-3 text-[10px] sm:text-xs mr-1 sm:mr-4" target="_blank" :href="`/linkedin/${item.id}/click`">
                  Odwiedź
                </a>
                <Link :href="`/linkedin/${item.id}/edit`" tabindex="-1" class="hidden sm:inline-block">
          ...
          <tr v-if="linkedin.data.length === 0">
            <td class="px-6 py-4 border-t text-center" colspan="5">Brak wpisów LinkedIn</td>
          </tr>
        </tbody>
      </table>
    </div>
